...
- **Card budgets vs insets** — the per-phase card sizes are measured against `controlInsetSide` and
  the bottom gesture strip, not the smaller `insetSide` the backgrounds use, because `BottomBand` —
  the container the cards actually sit in — is padded by the former. Budgeting on the latter
  overstated the column by twice the difference, and a `Row` hands a shortfall to its *last* child:
  the last card came out at half width while the rest stayed full size. That is what the "fifth
  card is too narrow" report was; the divisor change that preceded it treated a symptom. The cut's
  fixed overhead came down from 148dp to 52dp at the same time — the surplus had been paying for
  the inset nobody subtracted, and once the height stopped lying it was being charged twice.
...
`fastlane/metadata/android/en-GB/` holds everything the listing needs, in `supply`'s layout:
`title.txt`, `short_description.txt`, `full_description.txt`, `changelogs/<versionCode>.txt`, and
`images/` (icon, feature graphic, phone and tablet screenshots). The two graphics are generated by
`tools/generate-icon.py`; the screenshots are captured from emulators at exactly 16:9, which is what
Play asks for — 1920x1080 for phones and 2560x1440 for both tablet slots.

The description does **not** promise online play or Bluetooth. iOS offers both; this app connects
over the local network only, and a store listing is the wrong place to be aspirational.
